...
4. 现有三个本地 backup 分支（`backup/pre-doc-split`、`backup/pre-retitle`、`backup/pre-layout-split`）是 SHA 追溯用的，**保留**；`tmp/layout-split` 是纪元④ 重写的临时分支，可删可留（其内容与正式分支一致）；未经明确要求不要破坏旧对象、reflog 或 tags。
