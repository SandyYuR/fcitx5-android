...
> **【2026-09-16 状态注记】** 本报告的三个档位（A/B/C）最终**全部实施完毕**，含方案 C 的插件框架删除；与提交的逐项对应关系（`b1bba67b`→`7bc89b99`→`16d5444d`→`c86c32c3`→`dcc1046f`→`d85068c0`→`cc2f2d6b` 等）已写入 `docs/rime-integration-plan.md` 顶部状态注记。第 2.3 节"必须保留"清单中的 spell/quickphrase/imselector/unicode/androidkeyboard 被用户后续决策追加删除（见该节括号注），其余结论经实施验证成立。下文保留调研时点原文作为历史设计依据，其中行号、组件状态均为 `fx2@3ad25fc9` 基线的情况，不代表当前代码。
>
