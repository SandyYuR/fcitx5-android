...
2026-09-10 文档分拆：此前文档与代码混在 `fx-rime-only`，分拆时该分支已重写为纯代码历史（移除全部文档提交）。

**分拆前的完整历史**（184 个提交，含全部代码与文档提交，tip `b3da998e`）保存在 tag [`archive/pre-doc-split`](../../tree/archive/pre-doc-split)。交接文档中引用的旧提交 SHA 均属重写前历史，请在该 tag 下浏览；重写后的提交以 `fx-rime-only` 实测为准。
